...
const education = [
    {
        ...
        specialization: "Artificial Intelligence and Finance",
        coursework: ["Advanced Database Systems", "Internet Applications & Distributed Systems", "Advanced Software Engineering"],
        icon: "🎓",
    ...
    },
    {
        ...
        coursework: ["Predictive Analysis", "Android Development", "Cloud Computing", "Data Engineering", "DSA"],
        icon: "🏛️",
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    {education.map((edu, i) => (
                        <motion.div
                            ...
                            whileHover={{ y: -5 }}
                            className={`${edu.bgColor} rounded-3xl p-8 border border-white/60 shadow-xl hover:shadow-2xl transition-all duration-500 relative overflow-hidden`}
                        >
                            {/* Decorative corner gradient */}
                            <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${edu.color} opacity-10 rounded-bl-full`} />

                            <div className="relative z-10">
                                <div className="flex items-start justify-between mb-6">
                                    <span className="text-5xl">{edu.icon}</span>
                                    <span className={`px-4 py-1.5 text-xs font-bold rounded-full bg-gradient-to-r ${edu.color} text-white shadow-lg`}>
                                        ✓ Completed
                                    </span>
                                </div>

                                <h3 className="text-2xl md:text-3xl font-bold text-stone-800 mb-2">{edu.degree}</h3>
                                <p className={`text-xl font-semibold bg-gradient-to-r ${edu.color} bg-clip-text text-transparent`}>{edu.institution}</p>
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {edu.coursework.map((c) => (
                                            <span key={c} className="px-3 py-1.5 bg-white/80 text-stone-600 text-xs font-medium rounded-full border border-white shadow-sm">
                                                {c}
                                            </span>
                                        ))}
                                    </div>
